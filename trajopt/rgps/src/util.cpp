...
double kl_divergence(array_tf _p_K, array_tf _p_kff, array_tf _p_sigma_ctl,
    ...
    for(int i = 0; i < nb_steps; i++) {
        mat q_prec_ctl = inv_sympd(lsigma_ctl.slice(i));

        mat diff_K = (q_K.slice(i) - p_K.slice(i)).t() * q_prec_ctl * (q_K.slice(i) - p_K.slice(i));
        mat diff_crs = (q_K.slice(i) - p_K.slice(i)).t() * q_prec_ctl * (- q_kff.col(i) + p_kff.col(i));
        mat diff_kff = (- q_kff.col(i) + p_kff.col(i)).t() * q_prec_ctl * (- q_kff.col(i) + p_kff.col(i));

        kl += as_scalar(0.5 * log( det(q_sigma_ctl.slice(i)) / det(p_sigma_ctl.slice(i)) )
		                + 0.5 * trace(q_prec_ctl * p_sigma_ctl.slice(i))
		                - 0.5 * dm_act
...
py::tuple cubature_forward_pass(array_tf _mu_x0, array_tf _sigma_x0,
    ...
    for (int i = 0; i < nb_steps; i++) {
        ...
        chol_sigma_cubature.fill(0.0);
        chol_sigma_augmented.submat(0, 0, dm_state + dm_act - 1, dm_state + dm_act - 1) = chol(sigma_xu.slice(i), "lower");
        ...
        // propagate cubature points
         for (int j = 0; j < 2 * dm_augmented; j++){
            vec vec_xu = cubature_points.col(j);
            mat mat_xu = kron(vec_xu(span(0, dm_state + dm_act)).t(), eye(dm_state, dm_state))
            mat cov =  mat_xu * _sigma_dyn.slice(i) * mat_xu.t()
            output_cubature_points.col(j) = join_horiz(A.slice(i), B.slice(i), c.col(i), chol(cov, "lower")) * vec_xu;
        }
