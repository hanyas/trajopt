...
def main():
    ...
    for i in range(n_episodes):
        ...
        for n in range(time_steps):
            _obs = np.array([obs[0], np.cos(obs[1]), np.sin(obs[1]), obs[2], obs[3]])
            act = uref[..., n] + K[..., n] @ (_obs - xref[..., n]) + kff[..., n]
            obs, _, done, add_data = env.step(act * np.ones(1))
