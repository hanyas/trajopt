...
while t <= N:
    ...
    for k in range(N - t):
        ...
        Fk = F(x0=Xk, u=Uk)
        Xk_end = Fk['xf']
        Xk_end += MX([1, 1]) if t == 20 else MX([0, 0])
        J += Fk['qf']
    ...
    # x_init = [x1_opt[1] + np.random.randn()*0.01, x2_opt[1] + np.random.randn()*0.01]
    hist_x1_opt.append(x1_opt[0])
    hist_x2_opt.append(x2_opt[0])

    if t != N:
        x_init = [x1_opt[1], x2_opt[1]]
        hist_u_opt.append(u_opt[0])
    else:
        x_init = [1, 1]
        hist_u_opt = vertcat(DM.nan(1), hist_u_opt)
    t += 1
...
plt.figure(1)
plt.plot(t_grid, hist_x1_opt)
plt.plot(t_grid, hist_x2_opt)
plt.step(t_grid, hist_u_opt, c='r')
